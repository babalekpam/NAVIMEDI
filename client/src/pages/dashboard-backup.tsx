...
export default function Dashboard() {
  ...
  // Platform-wide stats for general context  
  const platformTenantBreakdown = {
    hospitals: 12,
    clinics: 18,
    pharmacies: 8,
    laboratories: 6,
  ...
  const renderTenantAdminDashboard = () => {
    ...
    const shellMetrics = {
      ...
      totalPatients: metrics?.monthlyClaimsTotal || 2847, // Using claims as proxy for patient activity
    ...
    return (
      <HospitalAdminShell metrics={shellMetrics} recentActivity={recentActivity}>
        {/* Original dashboard content wrapped in the new modern shell */}
        <div className="space-y-6">
          {/* Additional hospital admin controls */}
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Advanced Hospital Management</h2>
              <p className="text-gray-600 text-sm">Role-based permissions for {tenant?.name}</p>
            </div>
            <Button 
              variant="outline" 
              size="sm" 
  ...
  return user.role === 'super_admin' ? renderSuperAdminDashboard() : renderRoleSpecificDashboard();
}
