...
export default function PharmacyDashboardEnhancedV2() {
  ...
  const [ecoDeliveryModalOpen, setEcoDeliveryModalOpen] = useState(false);
  const [selectedEcoOption, setSelectedEcoOption] = useState('');
...
"AI Recommendations","4","Consider therapeutic substitutions for cost savings"`;
          ...
          <Card className="bg-gradient-to-br from-orange-50 to-red-50 border-orange-200">
            ...
            <CardContent>
              <div className="space-y-3">
                <button 
                  className="flex items-center justify-between w-full p-2 rounded hover:bg-orange-100 transition-colors text-sm"
                  onClick={() => {
                    alert('Active Deliveries:\n\n1. Order #DEL-2025-001 - Sarah Johnson\n   Status: In Transit\n   ETA: 2:30 PM\n   Driver: Mike Smith\n\n2. Order #DEL-2025-002 - Robert Davis\n   Status: Out for Delivery\n   ETA: 3:15 PM\n   Driver: Emily Chen\n\n3. Order #DEL-2025-003 - Maria Garcia\n   Status: Preparing\n   ETA: 4:00 PM\n   Driver: TBD');
                  }}
                >
                  <div>
                ...
                <button 
                  ...
                  onClick={() => {
                    ...
                    if (currentTime < 18) {
                      alert('Same-Day Delivery Available!\n\nCutoff Time: 6:00 PM\nDelivery Window: 2-4 hours\nCoverage Area: 15-mile radius\nDelivery Fee: $4.99\n\nOrders placed before 6 PM will be delivered today.');
                    } else {
                      alert('Same-Day Delivery Closed\n\nNext-Day Delivery Available:\nOrders placed now will be delivered tomorrow\nDelivery Window: 9 AM - 6 PM\nDelivery Fee: $2.99');
                    }
                  }}
                >
                  <div>
                ...
                <button 
                  className="flex items-center justify-between w-full p-2 rounded hover:bg-orange-100 transition-colors text-sm"
                  onClick={() => {
                    alert('Drone Delivery Service\n\nActive Drone Deliveries: 2\n\n1. Drone #DR-001\n   Patient: John Miller\n   Medication: Insulin\n   ETA: 25 minutes\n   Location: Downtown Area\n\n2. Drone #DR-002\n   Patient: Lisa Brown\n   Medication: Blood Pressure Meds\n   ETA: 12 minutes\n   Location: Suburban Area\n\nDrone delivery available within 5-mile radius for urgent prescriptions.');
                  }}
                >
                  <div>
                    ...
                    <div className="space-y-4 py-4">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                          <h4 className="font-medium text-green-800">Completed Today</h4>
                        ...
                        <div className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                          <h4 className="font-medium text-orange-800">Preparing</h4>
                      ...
                      <div className="space-y-3">
                        ...
                        {[
                          { id: 'DEL-001', patient: 'Sarah Johnson', status: 'Delivered', time: '1:45 PM', driver: 'Mike Smith' },
                          { id: 'DEL-002', patient: 'Robert Davis', status: 'In Transit', time: 'ETA 3:15 PM', driver: 'Emily Chen' },
                          { id: 'DEL-003', patient: 'Maria Garcia', status: 'Preparing', time: 'ETA 4:00 PM', driver: 'TBD' },
                          { id: 'DEL-004', patient: 'John Miller', status: 'Drone Delivery', time: 'ETA 25 min', driver: 'Drone DR-001' },
                        ].map((delivery) => (
                          <div key={delivery.id} className="flex items-center justify-between p-3 border rounded-lg">
                            <div>
                              <p className="font-medium">{delivery.patient}</p>
                              <p className="text-sm text-gray-600">Order {delivery.id}</p>
                            </div>
                            <div className="text-right">
                              <Badge className={
                                delivery.status === 'Delivered' ? 'bg-green-100 text-green-800' :
                                delivery.status === 'In Transit' ? 'bg-blue-100 text-blue-800' :
                                delivery.status === 'Drone Delivery' ? 'bg-purple-100 text-purple-800' :
                                'bg-orange-100 text-orange-800'
                              }>
                                {delivery.status}
                              </Badge>
                              <p className="text-xs text-gray-600 mt-1">{delivery.time}</p>
                      ...
                      <div className="flex gap-3 pt-4">
                        <Button 
                          onClick={() => alert('Refreshed delivery data from GPS tracking system!')}
                          className="flex-1"
                        >
                          <RefreshCw className="w-4 h-4 mr-2" />
                          Refresh Tracking
                ...
                {/* Eco-Friendly Delivery Option Selector */}
                <Dialog open={ecoDeliveryModalOpen} onOpenChange={setEcoDeliveryModalOpen}>
                  <DialogTrigger asChild>
                  ...
                  <DialogContent className="sm:max-w-lg">
                    ...
                    <div className="space-y-4 py-4">
                      ...
                      <div className="space-y-2">
                        <Label>Choose Eco-Friendly Option</Label>
                        <Select value={selectedEcoOption} onValueChange={setSelectedEcoOption}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select delivery method" />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="electric-bike">
                              🚴‍♂️ Electric Bike Delivery (0 emissions)
                            </SelectItem>
                            <SelectItem value="electric-vehicle">
                              🚗 Electric Vehicle (Low emissions)
                            </SelectItem>
                            <SelectItem value="drone-solar">
                              🔋 Solar-Powered Drone (0 emissions)
                            </SelectItem>
                            <SelectItem value="consolidated">
                              📦 Consolidated Route (Shared delivery)
                            </SelectItem>
                            <SelectItem value="carbon-neutral">
                              🌿 Carbon Neutral Delivery (Offset included)
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      ...
                      {selectedEcoOption && (
                        <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                          ...
                          {selectedEcoOption === 'electric-bike' && (
                            <div className="space-y-2 text-sm">
                              <p>✅ Zero emissions</p>
                              <p>🕒 Delivery time: 45-90 minutes</p>
                              <p>💰 Fee: $2.99</p>
                              <p>🌱 Carbon footprint: 0g CO2</p>
                            </div>
                          )}
                          {selectedEcoOption === 'electric-vehicle' && (
                            <div className="space-y-2 text-sm">
                              <p>✅ 80% less emissions than gas vehicles</p>
                              <p>🕒 Delivery time: 30-60 minutes</p>
                              <p>💰 Fee: $3.99</p>
                              <p>🌱 Carbon footprint: 50g CO2</p>
                            </div>
                          )}
                          {selectedEcoOption === 'drone-solar' && (
                            <div className="space-y-2 text-sm">
                              <p>✅ Solar-powered, zero emissions</p>
                              <p>🕒 Delivery time: 15-30 minutes</p>
                              <p>💰 Fee: $4.99</p>
                              <p>🌱 Carbon footprint: 0g CO2</p>
                            </div>
                          )}
                          {selectedEcoOption === 'consolidated' && (
                            <div className="space-y-2 text-sm">
                              <p>✅ Shared route reduces overall emissions</p>
                              <p>🕒 Delivery time: 2-4 hours</p>
                              <p>💰 Fee: $1.99</p>
                              <p>🌱 Carbon footprint: 75g CO2 (shared)</p>
                            </div>
                          )}
                          {selectedEcoOption === 'carbon-neutral' && (
                            <div className="space-y-2 text-sm">
                              <p>✅ Carbon offset included</p>
                              <p>🕒 Delivery time: 1-2 hours</p>
                              <p>💰 Fee: $5.99 (includes offset)</p>
                              <p>🌱 Net carbon footprint: 0g CO2</p>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                        ...
                        <p className="text-sm text-blue-700">
                          You've saved 2.4kg CO2 this month with eco-friendly deliveries!
                        </p>
                        <div className="mt-2 bg-blue-200 rounded-full h-2">
                          <div className="bg-blue-600 h-2 rounded-full w-3/4"></div>
                        </div>
                        <p className="text-xs text-blue-600 mt-1">75% towards your monthly eco goal</p>
                      </div>
                      
                      <div className="flex gap-3 pt-4">
                        <Button
                          onClick={() => {
                            if (deliveryAddress && selectedEcoOption) {
                              const optionName = selectedEcoOption.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase());
                              alert(`Eco-Friendly Delivery Scheduled!\n\nAddress: ${deliveryAddress}\nMethod: ${optionName}\n\nYour prescription will be delivered using our sustainable delivery option. Track your environmental impact in your profile.\n\n🌱 Thank you for choosing eco-friendly delivery!`);
                              setDeliveryAddress('');
                              setSelectedEcoOption('');
                              setEcoDeliveryModalOpen(false);
                            }
                          }}
                          className="flex-1 bg-green-600 hover:bg-green-700"
                          disabled={!deliveryAddress || !selectedEcoOption}
                        >
                          Schedule Eco Delivery
                        </Button>
                        <Button variant="outline" onClick={() => setEcoDeliveryModalOpen(false)}>
                          Cancel
                        </Button>
                      </div>
                    </div>
