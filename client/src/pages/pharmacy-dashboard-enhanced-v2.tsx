          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex gap-2">
                ...
                <button 
                  onClick={() => {
                    const content = `PHARMACY ANALYTICS REPORT
...
=== CURRENT PRESCRIPTIONS ===
${prescriptions.map(p => `${p.patientName} - ${p.medication} - Status: ${p.status} - Priority: ${p.priority}`).join('\n')}

=== INVENTORY ALERTS ===
${inventoryAlerts.map(a => `${a.medication} - Current Stock: ${a.currentStock} - Reorder Level: ${a.reorderLevel} - Urgency: ${a.urgency}`).join('\n')}
...
Report generated by NaviMED Pharmacy Management System
`;
                    ...
                    a.href = url;
                    a.download = `pharmacy-report-${selectedTimeframe}-${Date.now()}.txt`;
                    a.click();
                    URL.revokeObjectURL(url);
                  }}
