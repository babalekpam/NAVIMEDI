...
- **COMPREHENSIVE CROSS-TENANT INSTITUTION DISCOVERY:** Enhanced prescription and lab order forms with `/api/laboratories/all-available` and `/api/pharmacies/all-available` endpoints allowing hospitals/clinics to easily find and select from ALL available pharmacies and laboratories across tenants, added `preferredLaboratoryId` field to patient records, implemented security controls restricting access to healthcare organizations only (Aug 11, 2025)
- **PATIENT BOOKING JSON ERROR FIXED:** Resolved critical patient portal booking failure where API returned HTML instead of JSON by adding missing `/api/patient/appointments`, `/api/patient/profile`, and `/api/patient/book-appointment` endpoints with proper patient authentication and record matching (Aug 11, 2025)
